...
export const renderBillContent = (doc, data) => {
  ...
  doc
    .fontSize(11)
    .font("Courier-Bold")
    .text(restaurant.name.toUpperCase(), { align: "center" });
  doc
    .fontSize(8)
    .font("Courier")
    .text(restaurant.address || "", { align: "center" });
  if (restaurant.contact?.phone)
    doc.text(`Tel: ${restaurant.contact.phone}`, { align: "center" });
  if (restaurant.contact?.email)
    doc.text(`Email: ${restaurant.contact.email}`, { align: "center" });
  doc.moveDown(0.5);
  ...
  doc.text(
    `Receipt No : ${session.orderCode || session._id.toString().slice(-6)}`,
  );
  doc.text(`Date       : ${formatDateTime(session.endTime || new Date())}`);
  doc.text(
    `Table      : ${tableName}${tableLocation ? " - " + tableLocation : ""}`,
  );
  doc.text(
    `Cashier    : ${orders.find((o) => o.servedBy)?.servedBy?.fullName || "N/A"}`,
  );
  doc.moveDown(0.5);
  ...
  doc.text(`$${formatCurrency(subtotal)}`, 10, subtotalY, {
    width: 200,
    align: "right",
  });
  ...
  if (discountPercentage > 0) {
    ...
    doc.text(`$${formatCurrency(discountAmount)}`, 10, discountY, {
      width: 200,
      align: "right",
    });
  }
  ...
  doc.text(`${formatCurrency(grandTotal)}`, 10, totalY, {
    width: 200,
    align: "right",
  });
  doc.moveDown(0.5);
  ...
  if (
    ...
    restaurant.payosConfig?.accountHolder
  ) {
  ...
  if (session.startTime)
    doc.text(`Check-in  : ${formatTime(session.startTime)}`);
