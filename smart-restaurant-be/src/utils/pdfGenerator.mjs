...
export const generateSalesReportPDF = (res, data) => {
    ...
    const formatPeriod = () => {
        const labels = { today: 'Today', yesterday: 'Yesterday', week: 'Last 7 Days', month: 'Last 30 Days', custom: 'Custom Range' };
    ...
    doc.font('Helvetica-Bold').fontSize(18).fillColor('#1f2937')
       .text(restaurant.name, margin + 20, 155, { width: contentWidth - 40, align: 'center' });
    
    doc.font('Helvetica').fontSize(11).fillColor('#6b7280')
       .text(`Report Period: ${formatPeriod()}`, margin + 20, 178, { width: contentWidth - 40, align: 'center' })
       .text(`${formatDate(currentStart)} - ${formatDate(currentEnd)}`, margin + 20, 193, { width: contentWidth - 40, align: 'center' });
    ...
    doc.moveTo(margin, y + 18).lineTo(pageWidth - margin, y + 18).lineWidth(2).stroke('#10b981');
    
    y += 35;
    
    const metrics = [
        { label: 'Total Revenue', value: formatCurrency(currentMetrics.totalRevenue), trend: trends.revenue },
        { label: 'Total Orders', value: currentMetrics.totalOrders.toString(), trend: trends.orders },
        { label: 'Avg Order Value', value: formatCurrency(currentMetrics.avgOrderValue), trend: trends.avgOrderValue },
        { label: 'Avg Prep Time', value: `${Math.round(currentMetrics.avgPrepTime / 60)} min`, trend: trends.avgPrepTime }
    ];
    ...
    metrics.forEach((metric, i) => {
        ...
        doc.roundedRect(cardX, cardY, cardWidth, cardHeight, 6).lineWidth(1).stroke('#e5e7eb');
        ...
        doc.roundedRect(cardX + cardWidth - 70, cardY + 12, 55, 20, 4).fill(trendBgColor);
        doc.font('Helvetica-Bold').fontSize(9).fillColor(trendTextColor)
    ...
    doc.font('Helvetica').fontSize(8).fillColor('#9ca3af')
       .text(`Generated by Smart Restaurant System on ${new Date().toLocaleString('en-US')}`, 
              margin, pageHeight - 30, { width: contentWidth, align: 'center' });
    
    // Page border
    doc.rect(15, 15, pageWidth - 30, pageHeight - 30).lineWidth(1).stroke('#e5e7eb');
