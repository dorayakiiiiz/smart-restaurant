...
export const getOrdersForPrepTime = async (restaurantId, startDate, endDate) => {
...
export const calculateMetrics = (sessions, orders) => {
    ...
    const totalRevenue = sessions.reduce((sum, session) => sum + (session.totalAmount || 0), 0);
    ...
    orders.forEach(order => {
        if (order.acceptedAt && order.readyAt) {
            const prepTime = (new Date(order.readyAt) - new Date(order.acceptedAt)) / 1000; // seconds
